...
const textArea = document.querySelector(JVC_SEL.messageTopicInput);
const textAreaMP = document.querySelector(JVC_SEL.messagePrivateInput);

document.querySelector('.messageUser__actionIcon icon-pencil2')?.addEventListener('click', () => {
    setTimeout(() => {
        document.querySelectorAll(JVC_SEL.messageTopicInput).forEach(function (el) {
            console.log(el)
...
async function handleDrop(event) {
    ...
    console.log('[deboucled] drop event', { types: Array.from(dataTransfer.types), files: Array.from(dataTransfer.files) });
...
async function handlePaste(event) {
    ...
    console.log('[deboucled] paste event', { types: Array.from(clipboardData.types), files: Array.from(clipboardData.files) });
...
function updateTextArea(imageUrl, element) {
    ...
    const newValue = before + imageUrl + after;
